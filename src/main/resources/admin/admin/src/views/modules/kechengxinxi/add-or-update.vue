    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="班级" prop="banji">
          <el-select :disabled="ro.banji" @change="banjiChange" v-model="ruleForm.banji" placeholder="请选择班级">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.banji" label="班级" prop="banji">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.kejian" label="课件" prop="kejian">
          <file-upload
          tip="点击上传课件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.kejian?ruleForm.kejian:''"
          @change="kejianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.kejian" label="课件" prop="kejian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.kejian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/banjixinxi/banjibianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    banjiChange () {
      this.$http({
        url: `follow/banjixinxi/banjibianhao?columnValue=`+ this.ruleForm.banji,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
        } else {
	...
	if(this.ruleForm.kejian!=null) {
		this.ruleForm.kejian = this.ruleForm.kejian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    kejianUploadChange(fileUrls) {
	this.ruleForm.kejian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
